...
ave1234_lateral_data %>%
  ...
  shapiro_test(amplitude)   #normality 성립 안 하는데 일단 아노바 돌려보기 # residual normality # correction (log나 square root transform)
...
# Show comparison results for "Left" groups
pwc %>% filter(lmr == "Left") %>%
  select(-p)     # remove p columns

#lmr   .y.       group1 group2    n1    n2 p.adj p.adj.signif
#<chr> <chr>     <chr>  <chr>  <int> <int> <dbl> <chr>       
#  1 Left  amplitude High   Low      400   400 0.025 *         

# 왼쪽 전극들에서 high low 차이가 유의미 -> 근데 내 연구와는 무관
...
ave1234_midline_data %>%
  ...
  shapiro_test(amplitude)   #normality 성립!
